...
from model import PreActResNet18,SimpleDLA,Net_basic,MobileNetV2,PreActResNet34
import os
...
class Processdata(torch.utils.data.Dataset):
    ...
    def __getitem__(self,item):
        img0 = torch.from_numpy(self.data[0,item,:,:,:]).to(dtype=torch.float).unsqueeze(0).permute(0,3,1,2)
        img1 = torch.from_numpy(self.data[1,item,:,:,:]).to(dtype=torch.float).unsqueeze(0).permute(0,3,1,2)
